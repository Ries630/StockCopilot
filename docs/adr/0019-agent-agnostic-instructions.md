...
- 指示は `AGENTS.md` を正とし、`CLAUDE.md` は `@AGENTS.md` のインポートと Claude 固有の
  数行だけを持つ
...
ファイルが 1 つ減る。落としたのは Claude 固有の記述を置く場所が無くなるためで、
`/adr` skill と設計メモのパスは他のエージェントには意味がない。インポートなら
その下に書ける。
...
symlink と知らずに触ると混乱する。`CLAUDE.md` に「編集は実体側に対して行う」と
書いたが、これは文書でしか守られない。
